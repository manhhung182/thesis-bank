    ...
    if (extra.student !== undefined && extra.mssv !== undefined) {
      ...
      if (extra.student?.trim() && extra.mssv?.trim()) {
        ...
        if (existing) {
          ...
          await supabase.from('students').update({ full_name: extra.student, class: extra.class || null }).eq('id', studentId);
        } else {
          const { data: newS } = await supabase.from('students').insert({ full_name: extra.student, mssv: extra.mssv, class: extra.class || null }).select().single();
